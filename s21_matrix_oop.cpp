...
S21Matrix::S21Matrix(int32_t rows, int32_t cols) : rows_(rows), cols_(cols) {
  if (rows_ <= 0 || cols_ <= 0)
    throw std::length_error("Error: Array size cannot be zero");
...
bool S21Matrix::EqMatrix(const S21Matrix &other) const {
  if (rows_ != other.get_rows() || cols_ != other.get_cols()) return false;
...
void S21Matrix::SumMatrix(const S21Matrix &other) {
  if (rows_ != other.get_rows() || cols_ != other.get_cols())
    throw std::logic_error("Error: Matrices of different sizes");

  for (int32_t i = 0; i < rows_; ++i)
    for (int32_t j = 0; j < cols_; ++j) (*this)[i][j] += other[i][j];
}
...
void S21Matrix::MulNumber(const double num) {
  for (int32_t i = 0; i < rows_; ++i)
    for (int32_t j = 0; j < cols_; ++j) (*this)[i][j] *= num;
}

void S21Matrix::MulMatrix(const S21Matrix &other) {
  if (cols_ != other.get_rows() || rows_ != other.get_cols())
    throw std::logic_error("Error: Dimensions are not suitable");
...
S21Matrix S21Matrix::Transpose() const {
  ...
  for (int32_t i = 0; i < rows_; ++i)
    for (int32_t j = 0; j < cols_; ++j) res[j][i] = (*this)[i][j];
...
double S21Matrix::Determinant() const {
  if (this->rows_ != this->cols_)
    throw std::logic_error("Error: The matrix is not square");
...
S21Matrix S21Matrix::InverseMatrix() const {
  if (rows_ != cols_) throw std::logic_error("Error: The matrix is not square");

  double det = Determinant();
  if (std::fabs(det) < 1e-06)
    throw std::logic_error("Error: The determinant is zero");
...
int32_t S21Matrix::get_rows() const noexcept { return rows_; }
int32_t S21Matrix::get_cols() const noexcept { return cols_; }
...
void S21Matrix::set_rows(const int32_t &new_rows) {
  ...
  for (int32_t i = 0; i < (rows_ < new_rows ? rows_ : new_rows); ++i)
    for (int32_t j = 0; j < cols_; ++j) tmp[i][j] = (*this)[i][j];
...
double *S21Matrix::operator[](int32_t row) const {
  if (row >= rows_) throw std::out_of_range("Error: Incorrect input");
